    ...
    case Plane:
        {
            ...
            m_angularVelocity = 180*delta.length() / ( 3.1415926536*msecs);
            m_axis = QVector3D(-delta.dy(), delta.dx(), 0.0f).normalized();
            m_axis = transformation.rotatedVector(m_axis);
            m_rotation = QQuaternion::fromAxisAndAngle(m_axis, 180 / 3.1415926536 * delta.length()) * m_rotation;
        }
        break;
    case Sphere:
        {
            ...
            float angle = 180 / 3.1415926536 * asin(sqrt(QVector3D::dotProduct(m_axis, m_axis)));
